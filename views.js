            ...
            <div class="relative bg-white rounded-[2rem] shadow-2xl border border-slate-200 overflow-hidden w-[320px] md:w-[680px] flex flex-col max-h-[85vh]">
                <div class="p-6 bg-slate-50/50 border-b flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <button onClick=${() => navigate('year', 'prev')} aria-label="Oldingi yil" class="p-2 hover:bg-white rounded-xl shadow-sm border border-slate-100 active:scale-90 transition-all">
                            <${Lucide.ChevronLeft} size="16" />
                        </button>
                        <h3 class="text-xl font-black text-slate-800 tracking-tighter">${safeFormat(viewDate, 'yyyy')}</h3>
                        <button onClick=${() => navigate('year', 'next')} aria-label="Keyingi yil" class="p-2 hover:bg-white rounded-xl shadow-sm border border-slate-100 active:scale-90 transition-all">
        ...
        <div class="space-y-6 md:space-y-8 animate-fade-in pb-20" onClick=${() => { setActiveFilterCol(null); setShowColSettings(false); }}>
            <header class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div class="flex flex-wrap gap-3 w-full md:w-auto items-center">
                     <div class="relative">
                        <button onClick=${(e) => { e.stopPropagation(); setShowColSettings(!showColSettings); }} aria-label="Ustunlarni sozlash" class="bg-white text-slate-500 border border-slate-200 px-3 py-3.5 rounded-2xl hover:bg-slate-50 transition-all active:scale-95 shadow-sm" title="Ustunlarni sozlash">
                    ...
                    <div class="divide-y divide-slate-50">
                        ${deletedTasks.map(t => html`
                            <div key=${t.id} class="p-6 md:p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:bg-slate-50 transition-all group">
                                ...
                                <button onClick=${() => restoreTask(t.id)} aria-label="Vazifani tiklash" class="w-full md:w-auto bg-brand-50 text-brand-700 px-6 py-3 rounded-2xl text-[10px] font-black uppercase hover:bg-brand-900 hover:text-white transition-all shadow-sm active:scale-95">Tiklash</button>
                            </div>
                        `)}
                    </div>
                `}

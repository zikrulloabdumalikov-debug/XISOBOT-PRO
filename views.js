...
const MiniMonth = ({ monthDate, range, onSelect }) => {
    ...
    const getDayClass = (day) => {
        ...
        if (!isValid(start) || !isValid(end)) return 'text-slate-500';
        ...
        if (inInterval) return 'bg-brand-50 text-teal-800 font-bold'; // Better contrast
        return 'text-slate-600 hover:bg-slate-50 rounded-lg';
    };
    ...
    return html`
        <div class="bg-white p-3 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all cursor-default text-[9px] select-none">
            ...
            <div class="grid grid-cols-7 gap-0.5 text-center text-slate-400 font-bold mb-1">
            ...
            <div class="relative bg-white rounded-[2rem] shadow-2xl border border-slate-200 overflow-hidden w-[320px] md:w-[680px] flex flex-col max-h-[85vh]">
                <div class="p-6 bg-slate-50/50 border-b flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <button onClick=${() => navigate('year', 'prev')} aria-label="Oldingi yil" class="p-2 hover:bg-white rounded-xl shadow-sm border border-slate-100 active:scale-90 transition-all focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:outline-none">
                            <${Lucide.ChevronLeft} size="16" />
                        </button>
                        <h3 class="text-xl font-black text-slate-800 tracking-tighter">${safeFormat(viewDate, 'yyyy')}</h3>
                        <button onClick=${() => navigate('year', 'next')} aria-label="Keyingi yil" class="p-2 hover:bg-white rounded-xl shadow-sm border border-slate-100 active:scale-90 transition-all focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:outline-none">
                            <${Lucide.ChevronRight} size="16" />
                        </button>
                    </div>
                    <div class="flex bg-white p-1 rounded-xl border border-slate-200 shadow-inner">
                        <button onClick=${() => setViewMode('month')} class="px-4 py-1.5 text-[10px] font-black uppercase tracking-wider rounded-lg transition-all ${viewMode === 'month' ? 'bg-teal-700 text-white shadow-md' : 'text-slate-400'}">Oy</button>
                        <button onClick=${() => setViewMode('year')} class="px-4 py-1.5 text-[10px] font-black uppercase tracking-wider rounded-lg transition-all ${viewMode === 'year' ? 'bg-teal-700 text-white shadow-md' : 'text-slate-400'}">Yil</button>
                    </div>
                </div>
...
const ColumnSettings = ({ allColumns, hiddenColumns, setHiddenColumns, onClose }) => {
    ...
    return html`
        <div class="absolute top-full right-0 mt-2 w-56 bg-white rounded-xl shadow-2xl border border-slate-200 z-[70] p-3 animate-in fade-in zoom-in-95" onClick=${e => e.stopPropagation()}>
            ...
            <div class="space-y-1">
                ${allColumns.map(col => html`
                    <div key=${col.key} class="flex items-center justify-between px-2 py-1.5 hover:bg-slate-50 rounded-lg cursor-pointer" onClick=${() => toggleCol(col.key)}>
                        ...
                        <div class="w-8 h-4 bg-slate-200 rounded-full relative transition-colors ${!hiddenColumns.includes(col.key) ? 'bg-teal-600' : ''}">
            ...
            <button onClick=${onClose} class="w-full mt-3 py-2 bg-slate-100 text-slate-600 text-[10px] font-black uppercase rounded-lg hover:bg-slate-200 transition-colors focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:outline-none">Yopish</button>
        </div>
    `;
};
    ...
    return html`
        <div id="dashboard-content" class="space-y-6 md:space-y-10 animate-fade-in p-1 bg-[#f8fafc]">
            <header class="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-4 relative">
                <div><div class="flex items-center gap-2 mb-1"><span class="w-2 h-2 rounded-full bg-teal-500 animate-pulse"></span><p class="text-[10px] font-black text-teal-700 uppercase tracking-[0.3em]">Tizim Faol</p></div><h2 class="text-3xl md:text-4xl font-black text-slate-800 tracking-tight">Monitoring</h2></div>
                <div class="flex flex-col md:flex-row gap-3 w-full xl:w-auto items-center" data-html2canvas-ignore="true">
                    ${!isExporting && html`<div class="flex gap-2 w-full md:w-auto mr-2"><button onClick=${() => handleDownload('png')} aria-label="PNG yuklab olish" class="flex-1 md:flex-none justify-center bg-white text-slate-600 border border-slate-200 px-4 py-2.5 rounded-xl font-bold text-[10px] flex items-center hover:bg-slate-50 transition-all shadow-sm active:scale-95 uppercase tracking-wider focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:outline-none"><${Lucide.Image} size="16" class="mr-2 text-blue-500" /> PNG</button><button onClick=${() => handleDownload('pdf')} aria-label="PDF yuklab olish" class="flex-1 md:flex-none justify-center bg-white text-slate-600 border border-slate-200 px-4 py-2.5 rounded-xl font-bold text-[10px] flex items-center hover:bg-slate-50 transition-all shadow-sm active:scale-95 uppercase tracking-wider focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:outline-none"><${Lucide.FileDown} size="16" class="mr-2 text-red-500" /> PDF</button></div>`}
                    <div class="flex flex-col md:flex-row bg-slate-100 p-1.5 rounded-2xl gap-2 w-full md:w-auto shadow-inner relative">
                        ${preset !== 'jami' && html`<div class="flex items-center justify-between bg-white rounded-xl px-2 py-1 shadow-sm border border-slate-200/50 w-full md:w-auto"><button onClick=${() => handleNavigate('prev')} aria-label="Oldingi davr" class="p-2 hover:bg-slate-50 rounded-lg text-slate-500 transition-colors active:scale-90 focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:outline-none ${preset === 'ixtiyoriy' ? 'opacity-20 cursor-not-allowed' : ''}"><${Lucide.ChevronLeft} size="16" strokeWidth="3" /></button><div onClick=${() => setIsCalOpen(!isCalOpen)} class="group flex items-center justify-center cursor-pointer px-4 h-full min-w-[150px] select-none" role="button" aria-label="${"Sanani o'zgartirish: " + range.label}"><span class="text-xs font-black text-slate-700 uppercase tracking-wider whitespace-nowrap text-center group-hover:text-brand-500 transition-colors">${range.label}</span><${Lucide.Edit3} size="14" class="ml-2 text-slate-300 group-hover:text-brand-500 transition-all ${preset === 'ixtiyoriy' ? 'text-brand-500' : ''}" /></div><button onClick=${() => handleNavigate('next')} aria-label="Keyingi davr" class="p-2 hover:bg-slate-50 rounded-lg text-slate-500 transition-colors active:scale-90 focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:outline-none ${preset === 'ixtiyoriy' ? 'opacity-20 cursor-not-allowed' : ''}"><${Lucide.ChevronRight} size="16" strokeWidth="3" /></button></div>`}
                        ...
                        <div class="flex bg-white rounded-xl p-1 shadow-sm border border-slate-200/50 w-full md:w-auto overflow-x-auto gap-0.5">${['kun', 'hafta', 'oy', 'yil', 'ixtiyoriy', 'jami'].map(p => html`<button key=${p} onClick=${() => { setPreset(p); if(p!=='ixtiyoriy') setIsCalOpen(false); }} class="px-3 md:px-4 py-2 text-[9px] md:text-[10px] font-extrabold uppercase tracking-wider rounded-lg transition-all duration-300 focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:outline-none ${preset === p ? 'bg-teal-700 text-white shadow-md' : 'text-slate-500 hover:text-slate-600 hover:bg-slate-50'}">${p}</button>`)}</div>
                    </div>
                </div>
            </header>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6"><${KPICard} label="Bajarildi" count=${stats.done} icon="CheckCircle2" color="text-emerald-600" /><${KPICard} label="Jarayonda" count=${stats.doing} icon="Activity" color="text-amber-600" /><${KPICard} label="Rejada" count=${stats.todo} icon="Compass" color="text-indigo-600" /><${KPICard} label="Jami" count=${stats.total} icon="BarChart3" color="text-teal-600" /></div>
            <!-- Performance: Added [content-visibility:auto] (cv-auto) for off-screen rendering optimization -->
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8 pb-10 cv-auto">${['Bajarildi', 'Jarayonda', 'Rejada'].map(s => html`<div key=${s} class="bg-slate-100/50 p-4 md:p-6 rounded-[2rem] md:rounded-[2.5rem] border border-slate-200/50 min-h-[300px] md:min-h-[500px] flex flex-col"><div class="flex items-center justify-between mb-6 md:mb-8 px-2"><h3 class="text-[10px] md:text-[11px] font-extrabold text-slate-700 uppercase tracking-widest">${s}</h3><span class="bg-white text-[10px] md:text-[11px] font-black px-3 py-1 rounded-xl border border-slate-200 shadow-sm text-slate-700">${filtered.filter(t => t.status === s).length}</span></div><div class="space-y-4 md:y-5 flex-1 ${!isExporting ? 'overflow-y-auto custom-scrollbar max-h-[400px] md:max-h-none' : ''} pr-1 md:pr-2">${filtered.filter(t => t.status === s).map(t => html`<div key=${t.id} class="bg-white p-5 md:p-6 rounded-3xl shadow-sm border border-slate-100 active:scale-95 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 group cursor-pointer mb-4"><h4 class="font-bold text-slate-800 text-sm mb-4 leading-relaxed whitespace-normal break-words">${t.vazifa}</h4><div class="flex justify-between items-center text-[10px] font-bold text-slate-500 uppercase tracking-wider"><span class="flex items-center"><${Lucide.Calendar} size="12" class="mr-2 opacity-50" /> ${t.sana}</span><div class="flex items-center gap-2"><div class="w-12 h-1 bg-slate-100 rounded-full overflow-hidden"><div class="h-full bg-teal-500 transition-all duration-500" style=${{ width: t.progress + '%' }}></div></div><span class="text-teal-600">${t.progress}%</span></div></div></div>`)}</div></div>`)}</div>
        </div>
    `;
};
        ...
        <div class="space-y-6 md:space-y-8 animate-fade-in pb-20" onClick=${() => { setActiveFilterCol(null); setShowColSettings(false); }}>
            <header class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p class="text-xs text-slate-600 font-bold uppercase tracking-widest mt-1">
                ...
                <div class="flex flex-wrap gap-3 w-full md:w-auto items-center">
                     <div class="relative">
                        <button onClick=${(e) => { e.stopPropagation(); setShowColSettings(!showColSettings); }} aria-label="Ustunlarni sozlash" class="bg-white text-slate-500 border border-slate-200 px-3 py-3.5 rounded-2xl hover:bg-slate-50 transition-all active:scale-95 shadow-sm focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:outline-none" title="Ustunlarni sozlash">
                    ...
                    <button onClick=${() => exportToExcel(tasks)} aria-label="Excelga yuklash" class="flex-1 md:flex-none justify-center bg-white text-slate-600 border border-slate-200 px-6 py-3.5 rounded-2xl font-bold text-xs flex items-center hover:bg-slate-50 transition-all shadow-sm active:scale-95 focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:outline-none"><${Lucide.FileSpreadsheet} size="18" class="mr-2 text-emerald-500" /> Excel</button>
            ...
            <div class="bg-white rounded-3xl md:rounded-[2.5rem] border border-slate-200 shadow-xl overflow-hidden flex flex-col h-[65vh] md:h-[70vh]"><div class="overflow-x-auto overflow-y-auto custom-scrollbar flex-1 relative"><table class="w-full text-left text-[11px] border-separate border-spacing-0"><thead class="sticky top-0 z-20"><tr class="bg-slate-100 shadow-sm z-20">${visibleColumns.map(col => html`<th key=${col.key} class="${col.width} px-4 py-4 border-b border-slate-200 border-r border-slate-200/60 last:border-r-0 relative group select-none"><div class="flex items-center justify-between gap-2"><div onClick=${() => setSort(prev => ({ key: col.key, dir: prev.key === col.key && prev.dir === 'asc' ? 'desc' : 'asc' }))} class="flex items-center gap-1 cursor-pointer hover:text-brand-600 transition-colors uppercase font-extrabold tracking-widest text-[10px] text-slate-500 flex-1" role="button" aria-label="${col.label} bo'yicha saralash">${col.label}<${Lucide.ArrowUpDown} size="12" class="${sort.key === col.key ? 'text-brand-600 opacity-100' : 'opacity-30 group-hover:opacity-50'}" /></div><button onClick=${(e) => { e.stopPropagation(); setActiveFilterCol(activeFilterCol === col.key ? null : col.key); }} aria-label="${col.label} bo'yicha filtrlash" class="p-1 rounded-md hover:bg-slate-200 transition-colors ${filters[col.key] ? 'text-brand-500 bg-brand-50' : 'text-slate-300 hover:text-slate-500'} focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:outline-none"><${Lucide.Filter} size="14" fill=${filters[col.key] ? "currentColor" : "none"} /></button></div>${activeFilterCol === col.key && html`<${FilterDropdown} columnKey=${col.key} tasks=${tasks} activeFilters=${filters} onFilterChange=${handleFilterChange} onClose=${() => setActiveFilterCol(null)} />`}</th>`)}<th class="w-24 px-4 py-4 bg-slate-100 text-right border-b border-slate-200 border-l border-slate-200/60 uppercase font-extrabold tracking-widest text-slate-400 text-[10px] sticky right-0 z-30 shadow-[-10px_0_15px_-10px_rgba(0,0,0,0.05)]">Amallar</th></tr></thead><tbody class="divide-y divide-slate-100">${processedTasks.map(t => html`<tr key=${t.id} class="group hover:bg-slate-50 transition-all duration-200 ${t._isOverdue ? 'bg-red-50/50 hover:bg-red-50' : ''}">
                ...
                ${!hiddenColumns.includes('status') && html`<td class="px-4 py-3 border-r border-slate-50 align-top" onClick=${() => startCellEdit(t, 'status')}>${editCell.id === t.id && editCell.field === 'status' ? html`<select value=${cellValue} onChange=${e => saveCell(e.target.value)} autoFocus class="w-full px-2 py-1.5 bg-white border border-blue-400 rounded-lg text-xs font-bold"><option value="Rejada">Rejada</option><option value="Jarayonda">Jarayonda</option><option value="Bajarildi">Bajarildi</option></select>` : html`<span class="px-2.5 py-1 rounded-lg font-black uppercase text-[9px] block w-fit mt-1 ${t.status === 'Bajarildi' ? 'bg-emerald-50 text-emerald-700' : t.status === 'Jarayonda' ? 'bg-amber-50 text-amber-700' : 'bg-slate-100 text-slate-700'}">${t.status}</span>`}</td>`}
        ...
        <div class="space-y-6 md:space-y-8 animate-fade-in">
            <header class="flex justify-between items-center">
                ...
                <button 
                    ...
                    class="text-red-500 font-black text-[10px] md:text-[11px] uppercase tracking-[0.2em] hover:bg-red-50 px-4 md:px-6 py-2 md:py-3 rounded-2xl transition-all active:scale-95 focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:outline-none">
                    Tozalash
                </button>
            </header>
            ...
            <${Modal} isOpen=${isConfirmOpen} onClose=${() => setIsConfirmOpen(false)} title="Tozalashni tasdiqlang">
                <div class="text-center py-6">
                    ...
                    <div class="flex gap-4">
                        <button onClick=${() => setIsConfirmOpen(false)} class="flex-1 py-4 bg-slate-100 rounded-2xl font-black text-slate-500 uppercase tracking-widest text-xs focus-visible:ring-2 focus-visible:ring-slate-400 focus-visible:outline-none">Bekor qilish</button>
    ...
    return html`
        <div class="space-y-8 md:space-y-12 animate-fade-in pb-20">
            <header>
                ...
                <p class="text-xs text-slate-500 font-bold uppercase tracking-[0.2em] mt-2">Tizimdan professional foydalanish markazi</p>
            </header>
